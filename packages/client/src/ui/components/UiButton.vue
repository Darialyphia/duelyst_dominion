...
<template>
  ...
  >
    <Icon
      ...
      :icon="isLoading ? 'svg-spinners:90-ring-with-bg' : leftIcon!"
      aria-hidden="true"
    />
    ...
    <Icon v-if="rightIcon && !isLoading" :icon="rightIcon" aria-hidden="true" />
  </component>
</template>
...
@layer components {
  .ui-button {
    ...
    --_ui-button-hover-color: var(
      --ui-button-hover-color,
      var(--_ui-button-color)
    );
    --_ui-button-hover-bg: var(--ui-button-hover-bg, var(--_ui-button-bg));

    --_ui-button-focus-color: var(
      --ui-button-focus-color,
      var(--_ui-button-color)
    );
    --_ui-button-focus-bg: var(--ui-button-focus-bg, var(--_ui-button-bg));

    --_ui-button-disabled-color: var(
      --ui-button-disabled-color,
      var(--text-on-disabled)
    );
